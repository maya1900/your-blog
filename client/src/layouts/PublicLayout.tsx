      ...
      <header className="sticky top-0 z-40 backdrop-blur-md bg-surface/75 border-b border-whisper">
        <div className="max-w-[1280px] mx-auto px-6 md:px-10 h-16 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-8">
            <NavLink to="/" end viewTransition={false} className="nav-link">
              文章
            </NavLink>
            <NavLink to="/categories" viewTransition={false} className="nav-link">
              分类
            </NavLink>
            <NavLink to="/tags" viewTransition={false} className="nav-link">
              标签
            </NavLink>
            <NavLink to="/about" viewTransition={false} className="nav-link">
              关于
            </NavLink>
          </nav>
                  ...
                  <div className="absolute right-0 top-full pt-2 w-44 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition">
                    <div className="bg-surface border border-whisper rounded-lg py-2 shadow-sm">
                      <div className="px-3 py-1.5 border-b border-whisper">
                        ...
                        <p className="text-xs text-steel font-mono truncate">@{user.username}</p>
                      </div>
                      <Link
                        to="/me"
                        viewTransition={false}
